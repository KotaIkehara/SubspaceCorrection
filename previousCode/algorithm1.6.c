...
int main(int argc, char *argv[])
{
  ...
  for (ite = 1; ite < 5000; ite++)
  {
    ...
    /*--- end algorithm1 Selection of Approximate Solution Vectors---*/
  } //end iccg
  ...
  for (i = 0; i < m; i++)
  {
    for (k = 0; k < n; k++)
    {
      ...
      {
        jj = col_ind[j];
        ae[i * n + k] = ae[i * n + k] + val[j] * eq[i * n + jj];
      }
    }
  }
  //*--- end init ae ---*//
  ...
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, m_max, 1, n, 1.0, B, n, r, n, 0.0, f, 1);

  ab = (double *)malloc(sizeof(double) * n*m_max);
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, m_max, n, 1.0, val, n, B, n, 0.0, ab, n);
